...
// =======================================================
function ProdutoCard({ produto, noCarrinho, adicionarAoCarrinho, apiUrl, precoPromocional }) {
  ...
  useEffect(() => { setImgCarregada(false); }, [imgIndex]);

  const nextImg = (e) => { if (e) e.stopPropagation(); setImgIndex((prev) => (prev + 1) % fotos.length); };
  const prevImg = (e) => { if (e) e.stopPropagation(); setImgIndex((prev) => (prev === 0 ? fotos.length - 1 : prev - 1)); };
...
export default function HomeLoja() {
  ...
  const obterPrecoPromocional = (produto) => {
    const regraPromo = promocoesAtivas.find(p => p.nome === `PROMOCAO_${produto.categoria}`);
    if (!regraPromo) return null;

    let precoComDesconto = Number(produto.preco);
    const valorDesconto = Number(regraPromo.preco);

    if (regraPromo.referencia === 'PERCENTUAL') {
      precoComDesconto = precoComDesconto - (precoComDesconto * (valorDesconto / 100));
    } else {
  ...
  const temPromocaoNoCarrinho = useMemo(() => {
    return carrinho.some(i => i.categoria === 'UPSELL' || i.precoOriginal);
  }, [carrinho]);
  ...
  const calculosCarrinho = useMemo(() => {
    ...
    const totalFinal = Math.max(0, subtotalBruto - descontoCupom) + valorFrete;

    return { subtotalBruto, descontoCupom, valorFrete, totalFinal };
  }, [carrinho, cupomAtivo, temPromocaoNoCarrinho, freteSelecionado]);
  ...
  const produtosLoja = useMemo(() => {
    let filtrados = (produtos || []).filter(p => 
      (p.quantidade > 0 || p.categoria === 'LENTE') &&
      p.categoria !== 'UPSELL' && 
      p.categoria !== 'PROMO_CATE'
    );
  ...
  const processarPedido = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setPedidoFinalizado({ ...data.pedido, freteValor: calculosCarrinho.valorFrete, freteTipo: payload.freteTipo }); 
        setCarrinho([]); 
        setCupomAtivo(null);
        setCupomDigitado('');
        setFreteSelecionado(null);
          ...
          <div className="relative w-full max-w-[420px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 rounded-l-[2rem] overflow-hidden">
                          ...
                          <div key={item._id} className="flex items-center gap-4 bg-[#f9f8f6] p-4 rounded-2xl border border-transparent hover:border-[#c5a880]/30 transition-all group">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex gap-2 items-baseline">
                                ...
                                {item.precoOriginal && (
                                  <p className="text-[9px] text-gray-400 line-through font-bold">R$ {Number(item.precoOriginal).toFixed(2)}</p>
                                )}
                              </div>
                            </div>
                      ...
                      <div className="pt-6 mt-6 border-t border-gray-100">
                        ...
                        <div className="flex gap-2 mb-4">
                          ...
                          <button 
                            ...
                            disabled={calculandoFrete || cepFrete.length < 9}
                            className="px-5 bg-gray-200 text-gray-700 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-300 transition-colors disabled:opacity-50"
                          >
                            {calculandoFrete ? '⏳' : 'OK'}
                          </button>
                        </div>
                              ...
                              <label key={opcao.id} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${freteSelecionado?.id === opcao.id ? 'border-[#1d3026] bg-[#1d3026]/5 ring-1 ring-[#1d3026]' : 'border-gray-200 bg-white hover:border-[#c5a880]/50'}`}>
                                <div className="flex items-center gap-3">
                                  <input 
                                    ...
                                    className="w-4 h-4 text-[#1d3026] focus:ring-[#1d3026]"
                                  />
                                  <div className="flex flex-col">
                                    <span className="text-xs font-bold text-[#1d3026]">{opcao.nome}</span>
                                    <span className="text-[9px] text-gray-500 uppercase tracking-wider">{opcao.prazo}</span>
                                  </div>
                                </div>
                                <span className="text-xs font-black text-[#1d3026]">
                                  {opcao.valor === 0 ? 'GRÁTIS' : Number(opcao.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                </span>
                              </label>
                            ))}
                          </div>
                        ...
                        {temPromocaoNoCarrinho ? (
                          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest leading-relaxed">
                              🚫 Cupons não são cumulativos com os itens em liquidação que você já possui no carrinho.
                            </p>
                          </div>
                        ) : !cupomAtivo ? (
                    ...
                    <button 
                      ...
                      onClick={async () => { 
                        ...
                        try {
                          ...
                          const valorItensComDesconto = pedidoFinalizado.valorTotal - (pedidoFinalizado.freteValor || 0);
                          const fatorDesconto = subtotalOriginal > 0 ? valorItensComDesconto / subtotalOriginal : 1;
                          
                          const infiniteItems = pedidoFinalizado.itens.map(item => ({
                            quantity: 1,
                            price: Math.round((Number(item.preco) * fatorDesconto) * 100), 
                            description: item.nome
                          }));

                          if (pedidoFinalizado.freteValor > 0) {
                            infiniteItems.push({
                              quantity: 1,
                              price: Math.round(pedidoFinalizado.freteValor * 100),
                              description: `Frete: ${pedidoFinalizado.freteTipo}`
                            });
                          }
                          ...
                          const res = await fetch("https://api.checkout.infinitepay.io/links", {
                            ...
                            headers: { 
                              ...
                              "Authorization": `Bearer ${import.meta.env.VITE_INFINITEPAY_TOKEN || ''}`
                            },
            ...
            {etapaCheckout < 2 && carrinho.length > 0 && (
              <div className="p-6 md:p-8 border-t border-gray-100 bg-[#f9f8f6] space-y-4">
                ...
                {freteSelecionado && (
                  <div className="flex justify-between items-center text-gray-600 px-3 py-2">
                    <span className="text-[9px] font-black uppercase tracking-widest">Frete ({freteSelecionado.nome})</span>
                    <span className="text-xs font-bold">{freteSelecionado.valor === 0 ? 'GRÁTIS' : Number(freteSelecionado.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                  </div>
                )}
